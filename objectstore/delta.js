  ...
  return function(baseData, delta, dataType, callback) {
    ...
    while (stream.offset < stream.length) {
      ...
      if (opcode == 0) {
        ...
        bb.push(new Uint8Array(baseData.subarray(copyOffset, copyOffset + copyLength)).buffer)
        
      } else if ((opcode & 0x80) == 0) {
        bb.push(new Uint8Array(stream.data.subarray(stream.offset, stream.offset + opcode)).buffer)
