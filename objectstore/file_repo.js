Git.FileObjectStore = function(rootDir) {
 	this.dir = rootDir;
 	this.packs = [];
}

Git.FileObjectStore.prototype = {
	haveRefs: function(){
		return [];
	},
					    ...
					    if (entries.length){
							...
							if (packEntries.length){
								...
								packEntries.forEach(function(entry, i){
									Gito.FileUtils.readFile(packDir, entry.name, "ArrayBuffer", function(packData){
										var nameRoot = entry.name.substring(0, entry.name.lastIndexOf('.pack'));
										Gito.FileUtils.readFile(packDir, nameRoot + '.idx', 'ArrayBuffer', function(idxData){
											thiz.packs.push({pack: new Git.Pack(packData, thiz), idx: new Gito.PackIndex(idxData)});
											counter.x += 1;
	...
	_getHeadForRef : function(name, callback, onerror){
		Gito.FileUtils.readFile(this.dir, '.git/' + name, 'Text', function(data){callback(data.substring(0, 40));}, onerror) ;	
	},
	...
	_retrieveRawObject : function(sha, dataType, callback, error){
	     ...
	     if (sha instanceof Uint8Array){
	     	shaBytes = sha;
	     	sha = Gito.convertBytesToSha(shaBytes);
	     }
	     else{
	     	shaBytes = Gito.convertShaToBytes(sha);
	     }
		 ...
		 this._findLooseObject(sha, function(fileEntry){
		 	Gito.FileUtils.readFileEntry(fileEntry, 'ArrayBuffer', function(buf){
		 		var blob = RawDeflate.inflate(new Uint8Array(buf));
				Gito.FileUtils.readBlob(blob, dataType == 'Raw' ? 'ArrayBuffer' : dataType, function(data){
					callback(new Git.objects.RawLooseObject(data));
				});
		 	});
		 }, function(e){
	...
	_retrieveBlobsAsStrings : function(shas, callback){
		...
		shas.asyncEach(function(sha, done){
			self._retrieveRawObject(sha, 'Text', function(object){
				blobs.push(new Git.objects.Blob(sha, object.data));
				done();
			 });
		},
		function(){
	...
	_retrieveObject : function(sha, objType, callback){
		 ...
		 this._retrieveRawObject(sha, dataType, function(object){
		 	callback(new Git.objects[objType](sha, object.data), object);
		 });
	},
	...
	_init : function(success){
		var root = this.dir;
		var self = this;
		Gito.FileUtils.mkdirs(root, '.git/objects', function(objectsDir){
			self.objectsDir = objectsDir;
			Gito.FileUtils.mkfile(root, '.git/HEAD', 'ref: refs/heads/master\n', success);
		});
	...
	writeRawObject : function(type, content, callback){
		...
		bb.push(header);
		bb.push(new Uint8Array([0]).buffer);
		bb.push(content);
		var thiz = this;
		...
		fr.onloadend = function(e){
			...
			thiz._findPackedObject(Gito.convertShaToBytes(digest), function(){callback(digest);}, function(){
		...
		this.objectsDir.getDirectory(subDirName, {create:true}, function(dirEntry){
			dirEntry.getFile(objectFileName, {create:true}, function(fileEntry){
				fileEntry.file(function(file){
					 if(!file.size){
					 	var content = RawDeflate.deflate(store);
