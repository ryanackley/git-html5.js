...
//
//  parseAll()
Git.Pack = function(binary, repo) {
  //var binaryString = Git.toBinaryString(binary)
  var data;
  if  (binary.constructor == String)
  	data = new Uint8Array(Git.stringToBytes(binary));//new BinaryFile(binaryString)
  else
 ...
 this.getData = function(){
  ...
  var objectHash = function(type, content) {
    ...
    var data = Git.stringToBytes(type + " " + contentData.byteLength + "\0");
  ...
  var matchOffsetDeltaObject = function(header, dataType, success) {
    ...
    if (header.type == "ofs_delta"){
    	...
    	repo.objectStore._retrieveRawObject(sha, 'ArrayBuffer', function(baseObject){
    		baseObject.sha = sha;
    		expandDelta(baseObject.data, baseObject);
    	});
    }
  ...
  var expandOffsetDelta = function(baseObject, object, baseObjectData, objectData, dataType, callback) {
      ...
      Git.applyDelta(baseObjectData, objectData, dataType, function(expandedData){
...
Git.Pack.buildPack = function(heads, repo, callback){
	var visited = {};
	...
	var packIt = function(object){
		...
		if (object.compressedData){
			...
			if (buf instanceof ArrayBuffer){
				...
				data = Git.stringToBytes(buf);
			}
			compressed = RawDeflate.deflate(data);
			size = data.length;
		}
	...
	var finishPack = function(){
		...
		packedObjects.push(buf);
    //packed.reverse();
		for (var i = 0; i < packed.length; i++){
			packedObjects.push(packed[i]);
		}
		//packed.getBlob();
		Gito.FileUtils.readBlob(new Blob(packedObjects), 'ArrayBuffer', function(dataBuf){
			packed = null;
			var sha = Crypto.SHA1(new Uint8Array(dataBuf), {asBytes:true});
			
			var finalPack = [];//new BlobBuilder();
			finalPack.push(dataBuf);
			finalPack.push(new Uint8Array(sha).buffer);
			
			Gito.FileUtils.readBlob(new Blob(finalPack), 'ArrayBuffer', callback);		
		});
		//fr.readAsArrayBuffer(finalBlob);
		//Gito.FileUtils.readBlob(finalPack.getBlob(), 'ArrayBuffer', function(dataBuf){
		//	var sha = Crypto.SHA1(new Uint8Array(dataBuf), {asBytes:true});
	...
	var walkTree = function(treeSha, callback){
		...
		repo._retrieveObject(treeSha, 'Tree', function(tree, rawObj){
			...
			for (var i = 0; i < tree.entries.length; i++){
				var nextSha = Gito.convertBytesToSha(tree.entries[i].sha);
